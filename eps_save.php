...
if (isset($_POST['act'])){
  ...
  $act = $_POST['act'];
  //続いて、連想配列$_POSTにあるpid,reason項目を変数$pid, $reasonに代入
  $pid = $_POST['pid'];
  $reason = $_POST['reason'];

  if ($act=='update'){// 新規・編集($act)による場合分けでSQL文を作成
    $sql = "UPDATE tbl_wish SET pid='{$pid}',reason='{$reason}',updated_at=now() WHERE sid='{$sid}'";
    //$sql="INSERT INTO tbl_wish VALUES('{$uid}','{$pid}','{$reason}',now()); ";
  }else{
    $sql="INSERT INTO tbl_wish VALUES('{$sid}','{$pid}','{$reason}',now()); ";
    //$sql = "UPDATE tbl_wish SET pid='{$pid}',reason='{$reason}',updated_at=now() WHERE sid='{$uid}'";
  }
